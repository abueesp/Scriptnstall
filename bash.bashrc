...
alias sysmon="sudo dmidecode; lsb_release -a; uname -a; id; sudo id; sudo sysv-rc-conf; sudo lshw; lscpu; watch -n 2 free -m; logname; hostname; ipcs -m -c; sudo logname; sudo ipcs; sudo initctl list; systemctl status; cat /proc/uptime; sudo df -h;  sudo dmesg | less; ipcs -u; sudo service --status-all; sudo htop; sudo w -i; sudo dmidecode; sudo ps -efH | more; sudo lsof | wc -l; sudo lsof; ps aux | sort -nk +4 | tail; sudo pstree; sudo ss; sudo dpkg -l; sudo dstat -a -f; service --status-all; echo 'Check smb-nmb-telnet-rlogin-rexec-ftp-automount-named-lpd-lnetd-portmap, and if it is, that it is correctly configured with the last protocol version. To stop use -sudo pkill -9 service-, to avoid at boot -update-rc.d -f service-. To add 'update-rc.d service defaults'"
alias netmon="rfkill list; nmcli general; nmcli device; nmcli connection; curl ipinfo.io; netstat -lntup; sudo vnstat; sudo netstat -ie | more -s  -l -d -f; sudo netstat -s | more -s  -l -d -f; sudo sudo netstat -pt | more -s  -l -d -f; sudo tcpstat -i wlan0 -l -a; sudo iptables -S; sudo w -i; sudo ipcs -u; sudo tcpdump -i wlan0; sudo iotop; sudo ps; sudo netstat -r; echo 'En router ir a Básica -> Estado -> Listado de equipos; nmtui'"
alias portmon="sudo nc -l -6 -4 -u; sudo ss -o state established; sudo ss -l; sudo netstat -lp; sudo netstat -avnp -e"
