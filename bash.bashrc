...
##Linux container set
alias lxcinstall='sudo apt-get install lxc openssh-server -y; mkdir ~/.config/lxc; cp /etc/lxc/default.conf ~/.config/lxc/default.conf; echo "lxc.id_map = u 0 100000 65536" >> ~/.config/lxc/default.conf; echo "lxc.id_map = g 0 100000 65536" >> ~/.config/lxc/default.conf; printf "Current LXC uses the following kernel features to contain processes: 1. Kernel namespaces (ipc, uts, mount, pid, network and user) \n 2. Apparmor and SELinux profiles \n 3. Seccomp policies \n 4. Chroots (using pivot_root) \n 5. Kernel capabilities \n 6. CGroups (control groups)\n";'
alias lxcc='read -e -p "Name of the container (MYC by default): " -i "MYC" namecont; lxc-create -t download -n $namecont; read -e -p "Set a hard limit for kernel memory to fix fork bomb attack for cgroups (1000MB by default): " -i "1000" memm; read -e -p "And swap? (0MB by default): " -i "0" swapp; echo "lxc.cgroup.memory.limit_in_bytes = $memmM" >> sudo tee /var/lib/lxc/$namecont/config; summ=$(expr $swapp + $memm);  "lxc.cgroup.memory.memsw.limit_in_bytes = $summM; " >> tee /var/lib/lxc/$namecont/config && echo "Secured and monitoring in $namecont-debug.out"'
alias lxcs='read -e -p "Name of the container (MYC by default): " -i MYC namecont; lxc-start -n $namecont -F -l debug -o $namecont-debug.out && cat var/lib/lxc/$namecont/config && echo "Monitoring in $namecont-debug.out"'
